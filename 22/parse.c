...
static Type *declspec(Token **rest, Token *tok) // TODO
{
	if (equal(tok, "int"))
	{
		*rest = tok->next;
	...
	error_tok(tok, "error declspec %s %d", strndup(tok->loc, tok->len), tok->kind);
}

static Type *type_suffix(Token **rest, Token *tok, Type *ty) // TODO
{
	if (equal(tok, "("))
	{
		*rest = skip(tok->next, ")");
...
static Type *declarator(Token **rest, Token *tok, Type *ty) // TODO
{
...
static Node *declaration(Token **rest, Token *tok)
{
	...
	while (!equal(tok, ";"))
	{
		...
		Node *node = new_binary(ND_ASSIGN, lhs, rhs, start);
...
static Function *function(Token **rest, Token *tok) // TODO
{
	Type *basety = declspec(&tok, tok);
	Type *ty = declarator(&tok, tok, basety); // TODO
